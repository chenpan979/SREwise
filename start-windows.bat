...
echo [6/11] 启动 Monitor MCP 服务...
start "Monitor MCP Server" /min %PYTHON_CMD% mcp_servers/monitor_server.py
timeout /t 2 /nobreak >nul
echo [成功] Monitor MCP 服务已启动
echo.

REM 启动 K8s MCP 服务 (SREwise 新增)
echo [7/11] 启动 K8s MCP 服务...
start "K8s MCP Server" /min %PYTHON_CMD% mcp_servers/k8s_server.py
timeout /t 2 /nobreak >nul
echo [成功] K8s MCP 服务已启动
echo.

REM 启动 Alertmanager MCP 服务 (SREwise 新增)
echo [8/11] 启动 Alertmanager MCP 服务...
start "Alertmanager MCP Server" /min %PYTHON_CMD% mcp_servers/alertmanager_server.py
timeout /t 2 /nobreak >nul
echo [成功] Alertmanager MCP 服务已启动
echo.

REM 启动 Grafana MCP 服务 (SREwise 新增)
echo [9/11] 启动 Grafana MCP 服务...
start "Grafana MCP Server" /min %PYTHON_CMD% mcp_servers/grafana_server.py
timeout /t 2 /nobreak >nul
echo [成功] Grafana MCP 服务已启动
echo.

REM 启动 FastAPI 服务
echo [10/11] 启动 FastAPI 服务...
