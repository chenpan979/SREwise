...
async def load_mcp_tools_safe(
    ...
    """加载 MCP 工具；失败或超时返回空列表与可读错误信息，不向上抛出。

    Args:
        client: MCP 客户端
        timeout: get_tools 总超时（秒）。避免某个 MCP 服务不响应导致整个请求卡死。
    """
    try:
        ...
        return [], (
            f"MCP get_tools 在 {timeout}s 内未返回，已放弃并降级为仅本地工具。"
            "请检查每个 MCP 服务(端口/URL/transport)是否在线。"
        )
